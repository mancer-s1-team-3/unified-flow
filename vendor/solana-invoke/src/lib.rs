...
// ── VENDOR PATCH ──────────────────────────────────────────────────────────────
// On the BPF target this is unchanged from upstream. Off-chain (native test
// builds) we delegate to the runtime stub installed by `solana-program-test`
// instead of panicking, so Anchor CPIs execute and coverage can be measured.
