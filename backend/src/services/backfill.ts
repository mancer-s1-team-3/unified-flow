  ...
  while (true) {
    ...
    for (const sigInfo of signatures) {
      try {
        ...
        console.error(
          `Error in backfilling transaction ${sigInfo.signature}:`,
          err
        );
      }
    }
...
async function handleTokensClaimed(event: any, tx: any, signature: string) {
  ...
  console.log(
    `Backfilled withdrawal for stream ${streamId}: ${normalized.claimable.toString()} tokens`
  );
}
...
async function handleMilestoneUnlocked(event: any, tx: any, signature: string) {
  ...
  console.log(
    `Backfilled milestone unlock for stream ${streamId}: unlocked ${milestoneAmount.toString()} tokens`
  );
}
...
async function handleStreamCancelled(event: any, tx: any, signature: string) {
  ...
  console.log(
    `Backfilled cancel for stream ${streamId}: vested ${normalized.vestedAmount.toString()} tokens`
  );
}
