...
export const StreamDetailsDrawer = memo(function StreamDetailsDrawer({
  ...
  const isRecipientWallet =
    ...
    selectedStream.recipient?.toLowerCase() ===
      connectedWalletAddress.toLowerCase();
  const isCreatorWallet =
    connectedWalletAddress !== null &&
    selectedStream.creator?.toLowerCase() ===
      connectedWalletAddress.toLowerCase();
  const isCancelled =
    Number(selectedStream.status) === 3 || Boolean(selectedStream.cancelled);
  const cancelledTx = isCancelled
    ? selectedStream.transactions?.find((tx: any) => tx.type === "CANCEL") ??
      null
    : null;
  const cancelledAt =
    cancelledTx?.createdAt ?? selectedStream.updatedAt ?? null;
  const mintDecimals =
  ...
  const isMilestoneCompleted =
    ...
    (Number(selectedStream.completedAt || 0) > 0 ||
  ...
  const progressBase = isCancelled
    ? withdrawn
    : selectedStream.vestingType === 2
    ? unlocked
    : withdrawn;
  const progressPercent =
    total > 0 ? Math.min(Math.max((progressBase / total) * 100, 0), 100) : 0;
  const hasClaimable =
    selectedStream.vestingType === 2
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden pr-1">
          <div className="flex items-center justify-between gap-3 border-b border-zinc-900 pb-4 mb-5 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <h3 className="text-md font-extrabold text-zinc-100 truncate">
          ...
          {loadingDetails ? (
            <div className="flex flex-col items-center justify-center py-20 text-zinc-400 gap-2">
              ...
              <span className="text-xs">
                Fetching event signatures & slots...
              </span>
            </div>
          ) : (
            <>
              <div className="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-4 mb-5 min-w-0 overflow-hidden">
                ...
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span className="text-[10px] text-zinc-400 font-mono">
                    {isCancelled
                      ? `${formatTokenAmount(
                          selectedStream.withdrawn,
                          mintDecimals
                        )} / ${formatTokenAmount(
                          selectedStream.totalAmount,
                          mintDecimals
                        )} ${amountLabel} Cancelled`
                      : selectedStream.vestingType === 2
                      ? `${formatTokenAmount(
                          selectedStream.unlockedAmount || 0,
                          mintDecimals
                        )} / ${formatTokenAmount(
                          selectedStream.totalAmount,
                          mintDecimals
                        )} ${amountLabel} Unlocked`
                      : `${formatTokenAmount(
                          selectedStream.withdrawn,
                          mintDecimals
                        )} / ${formatTokenAmount(
              ...
              <div className="text-xs grid gap-3.5 bg-zinc-900/25 border border-zinc-900 p-4 rounded-2xl min-w-0 overflow-hidden">
                <div>
                  ...
                  <div className="grid gap-2 sm:flex sm:items-center sm:justify-between font-mono bg-zinc-950 border border-zinc-900 rounded-lg px-2.5 py-1.5 text-zinc-300 min-w-0 overflow-visible">
                    <span className="min-w-0 flex-1 break-all whitespace-normal select-all">
                      {selectedStream.id}
                    </span>
                    <button
                      type="button"
                      onClick={() =>
                        copyToClipboard(selectedStream.id, "drawer_id")
                      }
                  ...
                  */}
                  <div>
                    ...
                    <span className="font-semibold text-zinc-300 font-mono">
                      {selectedStream.milestoneCount} milestones
                    </span>
                  </div>
                </div>
                ...
                {isCancelled && cancelledAt && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-zinc-900/60 pt-3 min-w-0">
                    <div>
                      ...
                      <span className="font-semibold text-rose-300 break-words">
                  ...
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-zinc-900/60 pt-3 min-w-0">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-indigo-400 font-mono">
                          {formatTokenAmount(
                            Math.max(
                              Number(selectedStream.unlockedAmount || 0) -
                                Number(selectedStream.withdrawn),
                              0
                            ),
                            mintDecimals
                          )}{" "}
                    ...
                    <div className="border-t border-zinc-900/60 pt-3 min-w-0">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-2">
                        Milestones Allocation per Index
                      </span>
                        ...
                        {(() => {
                          const list = getMilestoneAllocations({
                            ...
                            milestoneCount: Number(
                              selectedStream.milestoneCount || 0
                            ),
                          ...
                          return list.map((amt: number, idx: number) => {
                            ...
                            return (
                              <div
                                key={idx}
                                className="flex items-center justify-between gap-3 font-mono bg-zinc-950 border border-zinc-900/70 rounded-xl px-3 py-2 text-zinc-300 text-[11px] min-w-0 overflow-hidden"
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <span
                                    className={`w-1.5 h-1.5 rounded-full ${
                                      isUnlocked
                                        ? "bg-emerald-450 shadow-[0_0_6px_rgba(16,185,129,0.5)]"
                                        : "bg-zinc-800"
                                    }`}
                                  />
                                ...
                                <div className="flex items-center gap-3 min-w-0 shrink-0">
                                  <span className="text-zinc-400 font-bold whitespace-nowrap text-right">
                                    {formatTokenAmount(amt, mintDecimals)}{" "}
                                    {amountLabel}
                                  </span>
                                  <span
                ...
                <div className="border-t border-zinc-900/60 pt-3 min-w-0">
                  ...
                  <span className="font-mono text-zinc-400 break-all block">
                    {selectedStream.creator}
                  </span>
                </div>
                ...
                <div className="border-t border-zinc-900/60 pt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] text-zinc-500 font-mono min-w-0">
                  <span className="flex items-center gap-1 min-w-0">
                    ...
                    <span className="min-w-0 break-words">
                      Start: {formatDate(selectedStream.startTs)}
                    </span>
                  </span>
                  {selectedStream.vestingType !== 2 && (
                    <span className="flex items-center gap-1 min-w-0">
                      <Calendar className="w-3 h-3 shrink-0" />{" "}
                      <span className="min-w-0 break-words">
                        End: {formatDate(selectedStream.endTs)}
                      </span>
                    </span>
                  )}
                  {selectedStream.vestingType === 2 &&
                    selectedStream.completedAt && (
                      <span className="col-span-1 sm:col-span-2 flex items-center gap-1 text-emerald-400 font-bold border-t border-zinc-900/40 pt-1.5 mt-1 min-w-0">
                        <Calendar className="w-3 h-3 shrink-0" />{" "}
                        <span className="min-w-0 break-words">
                          Completed At: {formatDate(selectedStream.completedAt)}
                        </span>
                      </span>
                    )}
                  {selectedStream.vestingType === 1 && (
                    <span className="col-span-1 sm:col-span-2 flex items-center gap-1 text-amber-500 font-bold border-t border-zinc-900/40 pt-1.5 mt-1 min-w-0">
                      <Calendar className="w-3 h-3 shrink-0" />{" "}
                      <span className="min-w-0 break-words">
                        Cliff Unlock: {formatDate(selectedStream.cliffTs)} (
                        {Number(selectedStream.cliffTs) -
                          Number(selectedStream.startTs)}
                        s duration)
                      </span>
                    </span>
                  )}
              ...
              <div className="mt-5 border-t border-zinc-900 pt-5 min-w-0">
                ...
                {!selectedStream.transactions ||
                selectedStream.transactions.length === 0 ? (
                  ...
                  <div className="grid gap-2.5 min-w-0">
                    {selectedStream.transactions.map((tx: any) => (
                      <div
                        key={tx.id}
                        className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-3 flex justify-between items-start gap-3 text-[10px] min-w-0 overflow-hidden"
                      >
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 mb-1">
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${
                                tx.type === "CREATE_STREAM"
                                  ? "bg-indigo-400"
                                  : "bg-emerald-400"
                              }`}
                            />
                          ...
                          <span className="text-[9px] text-zinc-500 font-mono block">
                            Signature: {shorten(tx.signature)}
                          </span>
                          <span className="text-[9px] text-zinc-500 font-mono block">
                            Slot: {tx.slot}
                          </span>
                        ...
                        <a
                          href={`https://solscan.io/tx/${
                            tx.signature
                          }?cluster=${getExplorerClusterParam(endpoint)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-indigo-400 hover:text-indigo-300 flex items-center gap-0.5 font-bold transition-all shrink-0"
                        >
                ...
                <>
                  {selectedStream.vestingType === 2 &&
                    (isCancelled || isEnded || paused ? (
                      ...
                      <button
                        onClick={() =>
                          prefillAction("unlock_milestone", selectedStream.id)
                        }
                        className="sm:col-span-2 flex items-center justify-center gap-1.5 bg-zinc-900 hover:bg-zinc-800 text-indigo-400 hover:text-indigo-300 border border-zinc-800 hover:border-zinc-700 py-2.5 rounded-xl transition-all"
                      >
                        <Unlock className="w-3.5 h-3.5" />
                    ...
                    <button
                      onClick={() => {
                        ...
                        const duration = Math.max(
                          Number(selectedStream.endTs || 0) -
                            Number(selectedStream.startTs || 0),
                          0
                        );
                        const cliffDuration = Math.max(
                          Number(selectedStream.cliffTs || 0) -
                            Number(selectedStream.startTs || 0),
                          0
                        );
                        const milestones = String(
                          selectedStream.milestones || ""
                        )
                          .split(";")
                ...
                <>
                  ...
                  {isCancelled || isEnded || paused ? (
                    ...
                    <>
                      <button
                        onClick={() =>
                          prefillAction("edit_linear", selectedStream.id)
                        }
                      ...
                      <button
                        onClick={() =>
                          prefillAction("edit_cliff", selectedStream.id)
                        }
                    ...
                    <button
                      onClick={() =>
                        prefillAction("edit_milestone", selectedStream)
                      }
                      disabled={unlocked > 0}
